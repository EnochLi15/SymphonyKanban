...
import { createExecution, listArtifacts, listExecutionsByIssue, recordArtifact, updateExecution, } from "./execution-store.js";
import { claimNextTodoIssue, getIssueById, getIssueByIdIncludingDeleted, listIssues, transitionIssueStatus, writeIssueEvent, } from "./issue-store.js";
import { getSchedulerSettings, updateSchedulerSettings, } from "./settings-store.js";
import { createTag, deleteTag, listTags, updateTag } from "./tag-store.js";
import { createWorkspace, findWorkspaceIdByLocalPath, listWorkspaces, updateWorkspace, } from "./workspace-store.js";
        ...
        res.json({
            data: rows
                ...
                if (!row || typeof row !== "object")
                    return null;
                const record = row;
                const localPath = (typeof record.local_path === "string" && record.local_path) ||
...
app.get("/workspaces/:id/deletion-check", (req, res) => {
    ...
    const row = db
        .prepare("SELECT COUNT(*) as count FROM issues WHERE workspace_id = ? AND deleted_at IS NULL")
        .get(id);
...
app.post("/tags", (req, res) => {
    const { name, type, color, rules, acceptanceCriteria, state, behavior, workflowDefinition, afterCreate, beforeRemove, } = req.body ?? {};
    if (!name || typeof name !== "string") {
        res.status(400).json({ error: "name required" });
        return;
    }
    const now = new Date().toISOString();
    const id = randomUUID();
    createTag(id, name.trim(), type ?? null, color ?? null, rules ?? null, acceptanceCriteria ?? null, state ?? null, behavior ?? null, workflowDefinition ?? null, afterCreate ?? null, beforeRemove ?? null, now);
    res.status(201).json({
        data: {
            id,
            name: name.trim(),
...
app.patch("/tags/:id", (req, res) => {
    ...
    const now = new Date().toISOString();
    updateTag(req.params.id, name.trim(), type ?? null, color ?? null, rules ?? null, acceptanceCriteria ?? null, state ?? null, behavior ?? null, workflowDefinition ?? null, afterCreate ?? null, beforeRemove ?? null, now);
    res.json({ ok: true });
});
app.delete("/tags/:id", (req, res) => {
    const row = db
        ...
        .get(req.params.id);
    if (row && BUILTIN_TAG_NAMES.includes(row.name)) {
...
app.delete("/issues", (_req, res) => {
    ...
    const tx = db.transaction(() => {
        db.prepare("UPDATE issues SET deleted_at = ?, updated_at = ? WHERE deleted_at IS NULL").run(now, now);
...
app.post("/issues", (req, res) => {
    ...
    const status = "Todo";
    const normalizedPriority = typeof priority === "number" ? priority : priority ? Number(priority) : null;
    ...
    const insertIssue = db.prepare(`INSERT INTO issues
     ...
     VALUES
      (?, ?, ?, ?, ?, ?, ?, ?)`);
    const insertEvent = db.prepare(`INSERT INTO issue_events
    ...
    const insertIssueTag = db.prepare("INSERT OR IGNORE INTO issue_tags (issue_id, tag_id) VALUES (?, ?)");
    const tx = db.transaction(() => {
        insertIssue.run(issueId, title.trim(), description ? String(description) : null, status, Number.isNaN(normalizedPriority) ? null : normalizedPriority, workspace_id, now, now);
        insertEvent.run(eventId, issueId, "created", JSON.stringify({
            status,
        ...
        for (const name of tagNames) {
            const existing = selectTag.get(name);
...
app.patch("/issues/:id", (req, res) => {
    ...
    const nextPriority = normalizedPriority === undefined ? existing.priority : normalizedPriority;
    const nextStatus = status !== undefined ? status : existing.status;
    const nextWorkspaceId = workspace_id !== undefined ? workspace_id : existing.workspaceId;
    const now = new Date().toISOString();
    const updateIssue = db.prepare(`UPDATE issues
    ...
    const insertIssueTag = db.prepare("INSERT OR IGNORE INTO issue_tags (issue_id, tag_id) VALUES (?, ?)");
    const tx = db.transaction(() => {
        updateIssue.run(nextTitle, nextDescription, nextStatus, nextPriority, nextWorkspaceId, now, id);
        if (tagNames !== null) {
            deleteIssueTags.run(id);
            for (const name of tagNames) {
                const existingTag = selectTag.get(name);
...
app.delete("/issues/:id", (req, res) => {
    ...
    const now = new Date().toISOString();
    try {
        db.prepare("UPDATE issues SET deleted_at = ?, updated_at = ? WHERE id = ?").run(now, now, id);
    }
    catch (error) {
        // eslint-disable-next-line no-console
...
app.post("/issues/:id/transition", (req, res) => {
    ...
    if (toStatus === "Done") {
        const executions = listExecutionsByIssue(issue.id);
        const latest = executions[0];
        if (!latest) {
            res.status(400).json({ error: "no execution" });
            return;
        }
        const rows = listArtifacts(latest.id);
...
app.post("/executions/:id/artifacts", (req, res) => {
    ...
    const now = new Date().toISOString();
    const id = randomUUID();
    const artifact = recordArtifact(id, req.params.id, type, typeof content === "string" ? content : null, typeof summary === "string" ? summary : null, now);
...
app.get("/review/:issueId", (req, res) => {
    ...
    const executions = listExecutionsByIssue(req.params.issueId);
    const latest = executions[0];
    if (!latest) {
